...
def sustitucionTrasera(M, syms):
    for i, row in reversed(list(enumerate(M))):
        ecn = -M[i][-1]
        for j in range(len(syms)):
            ecn += syms[j] * row[j]
        syms[i] = sp.solve(ecn, syms[i])[0]
    return syms
...
def validarSolucion(sistema, soluciones, tolerancia=1e-6):
    ...
    for eqn in sistema:
        ...
        assert eqn.subs(soluciones) < tolerancia
...
if __name__ == '__main__':
    ...
    #Varaibles simbólicas para las ecuaciones lineales.
    ...
    if matrizSusti.shape[0] != len(variablesX):
        print('Hay un infinito número de soluciones.')
